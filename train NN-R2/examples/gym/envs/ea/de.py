...
class DEEnv(gym.Env):
    def __init__(self):
        ...
        self.file = open('statistic_output-64-4-0110', 'a')
    ...
    def step(self, action):
        ...
        if np.fabs(self.best_value - self.best_so_far) < 1e-08:
            print("Best so fat = optima acheived before exhausting budget",self.best_so_far, self.best_value, self.budget)
            self.budget = 0
        if self.best_so_far < self.best_value:
            print("Error:",self.u[self.i], self.F1[self.i], self.best_so_far, self.best_value)
        
        #if np.any(self.F1[self.i] < self.best_value):
            #print(self.u[self.i], self.F1[self.i], self.best_so_far, self.info)
        ...
        # print("window and gen: ",time.time() - a)
        
        self.file.write('action = {} '.format(action))
        self.file.write('reward = {} \n'.format(reward))
        #if self.best_so_far < self.best_value:
            #print("error",self.i, self.u[self.i], self.F1[self.i], self.best_so_far, self.best_value) #, self.best_so_far)
        ...
        if self.i >= self.NP:
            ...
            #ob[19:23] = Success_Rate1(self.NP, self.n_ops, self.gen_window, 1, self.max_gen); # print(self.ob[19:23])
            #ob[23:27] = Success_Rate1(self.NP, self.n_ops, self.gen_window, 2, self.max_gen)
            #ob[27:31] = Success_Rate1(self.NP, self.n_ops, self.gen_window, 3, self.max_gen)
            #ob[31:35] = Success_Rate1(self.NP, self.n_ops, self.gen_window, 4, self.max_gen)
            
            #ob[35:39] = Weighted_Offspring1(self.NP, self.n_ops, self.gen_window, 1, self.max_gen)
            #ob[39:43] = Weighted_Offspring1(self.NP, self.n_ops, self.gen_window, 2, self.max_gen)
            #ob[43:47] = Weighted_Offspring1(self.NP, self.n_ops, self.gen_window, 3, self.max_gen)
            #ob[47:51] = Weighted_Offspring1(self.NP, self.n_ops, self.gen_window, 4, self.max_gen)
            
            #ob[51:55] = Best_Offspring1(self.NP, self.n_ops, self.gen_window, 1, self.max_gen)
            #ob[55:59] = Best_Offspring1(self.NP, self.n_ops, self.gen_window, 2, self.max_gen)
            #ob[59:63] = Best_Offspring1(self.NP, self.n_ops, self.gen_window, 3, self.max_gen)
            #ob[63:67] = Best_Offspring1(self.NP, self.n_ops, self.gen_window, 4, self.max_gen)
            
            #ob[67:71] = Best_Offspring2(self.NP, self.n_ops, self.gen_window, 1, self.max_gen)
            #ob[71:75] = Best_Offspring2(self.NP, self.n_ops, self.gen_window, 2, self.max_gen)
            #ob[75:79] = Best_Offspring2(self.NP, self.n_ops, self.gen_window, 3, self.max_gen)
            #ob[79:83] = Best_Offspring2(self.NP, self.n_ops, self.gen_window, 4, self.max_gen)
            ...
            #print("inside: ",self.copy_ob)
            #else:
                #self.ob[19:83] = np.zeros(64)
        ...
        #print("outside: ",type(self.copy_ob), self.copy_ob)
        ...
        #self.ob[5] = self.jrand; self.ob[5] = self.ob[5] / self.dim; #print(self.ob[5])
        ...
        #self.ob[6] = distance.euclidean(self.population[self.i], self.population[self.r[0]]) / self.max_dist; #print(self.ob[5])
        #self.ob[7] = distance.euclidean(self.population[self.i], self.population[self.r[1]]) / self.max_dist;
        #self.ob[8] = distance.euclidean(self.population[self.i], self.population[self.r[2]]) / self.max_dist;
        #self.ob[9] = distance.euclidean(self.population[self.i], self.population[self.r[3]]) / self.max_dist;
        #self.ob[10] = distance.euclidean(self.population[self.i], self.population[self.r[4]]) / self.max_dist;
        ...
        #self.ob[12] = np.fabs(self.copy_F[self.r[0]] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far); #print(self.ob[11])
        #self.ob[13] = np.fabs(self.copy_F[self.r[1]] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far);
        #self.ob[14] = np.fabs(self.copy_F[self.r[2]] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far);
        #self.ob[15] = np.fabs(self.copy_F[self.r[3]] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far);
        #self.ob[16] = np.fabs(self.copy_F[self.r[4]] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far);
        # best parent in parent population
        #self.ob[16] = distance.euclidean(self.population[self.i], self.population[self.best]) / self.max_dist;
        #self.ob[17] = np.fabs(self.copy_F[self.best] - self.copy_F[self.i]) / (self.worst_so_far - self.best_so_far); #print(self.ob[17])
        ...
        self.file.write(','.join([str(i) for i in ob])+'\n')
        if self.budget <= 0:
