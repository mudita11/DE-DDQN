...
class TrainEpisodeLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        if np.mean(self.rewards[episode]) > self.lastreward:
            ...
            self.lastreward = np.mean(self.rewards[episode])
            ...
            newWeights = 'checkpoint_reward_{}.h5f'.format(np.mean(self.rewards[episode]))
